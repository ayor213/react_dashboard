...
const BarChart = ({
    ...
    useEffect(() => {
        if (d3Container.current && !d3Container.current.querySelector('svg')){
            ...
            const organizationData = Array.from(organizations, ([key, value]) => ({
                orgCode: key,
                admissions: value
            }));
            ...
            const yScale = d3.scaleLinear()
                .range([h-margin.bottom-10, 0]);

            // Add x-axis
            const xAxis = svg.append('g')
                .attr("class", "x-axis")
                .attr("transform", `translate(70,${h -margin.bottom})`)
                .call(d3.axisBottom(xScale))
                .attr("font-size", ".9rem");
            xAxis.append("text")
                .text("Hospital Codes")
                .attr("x", h / 2 - margin.bottom)
                //.attr("y", 780)
                //.attr('transform', 'translate(550,780)')
                .style("fill", `${colors.primary[100]}`)
                .attr("font-size", 30)
           ...
           // Append the dropdown within the svg
...
const dropdown = dropdownContainer.append('select')
            ...
            function update() {
                ...
                const yearData = Object.entries(admissionsByYearAndOrg[selectedYear]).map(([orgCode, admissions]) => ({
                    orgCode,
                    admissions
                }));
                // Sort data
                ...
                *******************************/
                ...
                barz.enter()
                    ...
                    .attr("height", d => h -margin.bottom - 5 - yScale(d.admissions))
